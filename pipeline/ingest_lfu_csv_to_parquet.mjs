...
async function ingestGroup({ csvFiles, outRawParquet, outDailyParquet, startDate }) {
  if (csvFiles.length === 0) throw new Error('No files')

  ensureDir(path.dirname(outRawParquet))
  ...
  for (const csvPath of csvFiles) {
    let inTable = false
    let headerLine = ''
    await forEachLine(csvPath, async (line0) => {
      ...
      if (!inTable) {
        if (line.startsWith('Datum;')) {
          ...
          if (!rawWriter) {
            rawWriter = await parquet.ParquetWriter.openFile(
              ...
              outRawParquet,
              { useDataPageV2: false, rowGroupSize: 50_000 }
            )
          }
          return
        }
  ...
  if (rawWriter) await rawWriter.close()
...
async function main() {
  ...
  if (levelFiles.length > 0) {
    station = await ingestGroup({
      csvFiles: levelFiles,
      outRawParquet: path.join(rawDir, `station_${args.stationId}_water_level_cm.parquet`),
  ...
  if (tempFiles.length > 0) {
    station =
      (await ingestGroup({
        csvFiles: tempFiles,
        outRawParquet: path.join(rawDir, `station_${args.stationId}_water_temperature_c.parquet`),
